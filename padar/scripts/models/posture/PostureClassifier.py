...
class PostureClassifier(BaseModel):
    def __init__(self, verbose, feature_set, class_set):
        ...
        self._name = "posture and activity classifier"
        self._description = "AR model trained on acceleromete data from multiple sensors to classify three postures"
    ...
    def _preprocess(self, train_df, class_df):
        ...
        class_set = class_df.loc[:, "posture"].values
    ...
    def _test(self, test_df, **kwargs):
        ...
        pred_df['POSTURE_PREDICTION'] = pred_set
        if kwargs['prob'] in ['True', '1', 1, True]:
            pred_df['POSTURE_PREDICTION_PROB'] = pred_max_prob_set
