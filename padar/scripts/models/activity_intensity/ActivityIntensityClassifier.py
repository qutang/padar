...
def init(verbose, feature_set, class_set):
    return ActivityIntensityClassifier(verbose, feature_set, class_set)

class ActivityIntensityClassifier(BaseModel):
    def __init__(self, verbose, feature_set, class_set):
        BaseModel.__init__(self, verbose, feature_set, class_set)
        self._name = "Activity intensity classifier"
        self._description = "AR model trained on acceleromete data from Non-dominant wrist to classify sedentary, light, moderate and vigorous activity intensity levels for adults"
    ...
    def _preprocess(self, train_df, class_df):
        ...
        class_set = class_df.loc[:, "activity_intensity"].values
    ...
    def _test(self, test_df, **kwargs):
        ...
        pred_df['activity_intensity_PREDICTION'] = pred_set
        if kwargs['prob'] in ['True', '1']:
            pred_df['activity_intensity_PREDICTION_PROB'] = pred_max_prob_set
