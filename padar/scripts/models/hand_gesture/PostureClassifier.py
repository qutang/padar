...
def init(verbose, feature_set, class_set):
    return HandGestureClassifier(verbose, feature_set, class_set)

class PostureClassifier(BaseModel):
    def __init__(self, verbose, feature_set, class_set):
        BaseModel.__init__(self, verbose, feature_set, class_set)
        self._name = "hand gesture classifier"
        self._description = "AR model trained on accelerometer data from multiple sensors to classify hand gestures"
    ...
    def _preprocess(self, train_df, class_df):
        ...
        class_set = class_df.loc[:, "hand_gesture"].values
    ...
    def _test(self, test_df, **kwargs):
        ...
        pred_df['HAND_GESTURE_PREDICTION'] = pred_set
        if kwargs['prob'] in ['True', '1', 1, True]:
            pred_df['HAND_GESTURE_PREDICTION_PROB'] = pred_max_prob_set
